...
class Service:
    ...
    def get_url(self, path):
        ...
        if host.startswith("http"):
            ...
            url = "http://{}/{}".format(host.rstrip("/"), path)
        return url

    def stop_if_error_occured(self, response):
        if response.status_code != 200:
            sys.stderr.write(
                "server rejected meta data."
    ...
    def post_file(self, rid, filename, filetype, compress=False):
        ...
        if "vmprof.com" in self.host:
            ...
            if size >= 100 * 1000 * 1000:  # 100 MB
                warnings.warn(
                    "WARNING: vmprof.com limits the "
                    "compressed profile file to 100 MBs. "
                    "The upload might fail!\n"
                )
        with open(filename, "rb") as fd:
            url = self.get_url(f"/api/runtime/upload/{filetype}/{rid}/add")
            headers = self.get_headers()
    ...
    def post(self, kwargs):
        ...
        if Service.FILE_CPU_PROFILE in kwargs:
            ...
            if os.path.exists(filename):
                ...
                self.post_file(rid, filename, Service.FILE_CPU_PROFILE, compress=False)
        ...
        if Service.FILE_JIT_PROFILE in kwargs:
            ...
            if os.path.exists(filename):
                ...
                response = self.post_file(
                    rid, filename, Service.FILE_JIT_PROFILE, compress=True
                )
    ...
    def finalize_entry(self, rid, data=b""):
        ...
        if response.status_code != 200:
            sys.stderr.write(
                "server failed to freeze these runtime profiles."
                " status: %d, msg: '%s'\n" % (response.status_code, response.text)
            )
            return False
        return True
