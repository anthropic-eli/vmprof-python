...
import vmprof
from vmprof.stats import AssemblerCode, JittedCode, Node, Stats
...
def test_tree_basic():
    ...
    tree = stats.get_tree()
    assert tree == Node(1, "foo", 2, {2: Node(2, "bar", 1), 3: Node(3, "baz", 1)})
...
def test_read_simple():
    ...
    stats = vmprof.read_profile(
        path, virtual_only=True, include_extra_info=True, lib_cache=lib_cache
    )
    tree = stats.get_tree()
    ...
    assert tree["foo"]["bar"].jitcodes == {140523638275600: 27, 140523638276656: 3}
