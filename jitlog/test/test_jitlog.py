...
from jitlog.parser import ParseContext, ParseException, _parse_jitlog
from vmprof.test.test_reader import FileObj
from vmprof.test.test_run import BufferTooSmallError, FileObjWrapper
from vmshare.binary import encode_le_u32, encode_le_u64, encode_str
...
def test_read_resop():
    fobj = FileObj(
        [
            const.MARK_RESOP_META + b"\x02\x00",
...
def test_asm_positions():
    ...
    assert (
        str(forest.traces[0xFFAA].get_stage("noopt").ops[0])
        == "i3 = fire(i2, i1, @descr())"
    )
...
def test_counters():
    ...
    fobj = FileObj(
        [
            ...
            b"l",
            encode_le_u64(5),  # not counted to 0xabcdef
        ]
    )
    fw = FileObjWrapper(fobj)
    forest = construct_forest(fw)
...
def test_merge_point_encode():
    ...
    parta = (
        b"\x22\x19\x00\x00\x00jitlog/test/data/code2.py"
        b"\x01\x00"
        b"\x03\x00\x07\x13\x00\x00\x00self.unique = False"
    )
    partb = (
        b"\x22\x18\x00\x00\x00jitlog/test/data/code.py"
        b"\x03\x00"
...
def test_iter_ranges():
    ...
    assert list(iter_ranges([0, 1, 100, 101, 102, 300, 301])) == [
        r(0, 2),
        r(100, 103),
        r(300, 302),
    ]
...
def test_read_jitlog_counter():
    ...
    assert (
        marks.read_jitlog_counter(ParseContext(forest), None, fw) == False
    ), "must not find trace"
    fw = FileObjWrapper(
        FileObj(
            [
...
def test_32bit_log_header():
    fobj = FileObj([const.MARK_JITLOG_HEADER + b"\x01\x00\x01" + encode_str("ppc64le")])
...
def test_exception_recover():
    ...
    fobj = FileObj(
        [const.MARK_JITLOG_HEADER, b"\x01\x00\x00", encode_str("x86_64"), b"\x00"]
    )
